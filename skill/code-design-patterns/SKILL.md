...
description: Choose, apply, and critique software design patterns — GoF (Strategy, Factory, Observer, State, Decorator, Adapter, Command, and the rest), architectural patterns (dependency injection, repository, hexagonal/ports-and-adapters, layered, CQRS, event sourcing), concurrency patterns (worker pool, pipeline, fan-out/fan-in, actor, producer-consumer), distributed resilience patterns (circuit breaker, retry with backoff, bulkhead, saga, outbox, idempotency key, rate limiter), and frontend component patterns (compound components, container/presentational, custom hooks, provider). Use this skill whenever the work involves object-oriented or component design, class modelling, low-level design (LLD) or machine-coding interview problems, "design a parking lot / elevator / rate limiter / vending machine"-style tasks, refactoring toward SOLID, reviewing or critiquing someone's class design, deciding how to structure a new module or service, or any question that mentions design patterns, code architecture, extensibility, coupling, or code smells — even when the user does not say the words "design pattern". Also use it to grade or self-check a design against a rubric before presenting it.
license: MIT
---
