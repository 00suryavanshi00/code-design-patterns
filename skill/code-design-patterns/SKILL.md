...
| State or load exceeds one node, or a legacy system must be replaced incrementally | Sharding and consistent hashing, Strangler Fig | `references/04-distributed-resilience-patterns.md` |
...
| `references/04-distributed-resilience-patterns.md` | Circuit breaker, Retry+jitter, Hedged requests, Timeout budgets, Bulkhead, Rate limiting (token/leaky bucket), Saga, Outbox, Idempotency, Expand–contract, Distributed locks and fencing tokens, Sharding and consistent hashing, Caching, API Gateway/BFF, Sidecar, Strangler Fig, Leader election, Dead letter queue | Service boundaries, network calls |
| `references/05-frontend-patterns.md` | Container/presentational, Custom hooks, Compound components, Provider, Render props, HOC, State reducer, Error boundary, Optimistic UI updates, Feature-sliced structure | React/Vue/UI component design |
...
| `references/09-operability.md` | Test doubles and when a fake beats a mock, testing time and concurrency, test levels, RED and USE, how to observe and what each pattern costs to run | "How would you test this?"; any design using a breaker, queue, pool or cache |
